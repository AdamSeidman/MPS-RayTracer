...
int main( int argc, char* argv[] ) 
{
    ...
    // Properties for dynamic method calculated
    DynamicProperties dprops;

    if (data.partitioningMode == PART_MODE_DYNAMIC)
    {
        dprops.numXUnits = (data.width / data.dynamicBlockWidth);
        if ( (data.width % data.dynamicBlockWidth) != 0 )
        { ++dprops.numXUnits; }

        dprops.numYUnits = (data.height / data.dynamicBlockHeight);
        if ( (data.height % data.dynamicBlockHeight) != 0 )
        { ++dprops.numYUnits; }

        dprops.numberUnits = dprops.numXUnits * dprops.numYUnits;
    }
    ...
    if ( data.mpi_rank == 0 )
    {
        ...
        masterMain( &data, &dprops );
    }
    else
    {
        // Start worker threads for ray tracer.
        slaveMain( &data, &dprops );
    }
